...
export default async function handler(req: any, res: any) {
  ...
  try {
    const [tokenMetadata, curveInfo, isGraduated, graduationStatus, poolAddress, price] = await Promise.all([
      client.readContract({
      ...
      client.readContract({
        ...
        args: [id as `0x${string}`, BigInt('1000000000000000000')],
      }).catch(() => null),
    ]);
    ...
    const tokenData = {
      ...
      virtualSomiReserves: formatEther(curveInfo[2]),
      virtualTokenReserves: formatEther(curveInfo[3]),
      graduated: isGraduated,
      canGraduate: graduationStatus ? graduationStatus[0] : false,
      poolAddress: poolAddress || null,
      price: price ? formatEther(price as bigint) : '0',
      tradingLink: `https://tradesomnia.fun/token/${id}`,
    };
    ...
    console.error('Error in tokens/[id] API:', error);
