...
async function getTokenData(tokenAddress: string) {
  try {
    const [tokenMetadata, curveInfo, isGraduated] = await Promise.all([
      client.readContract({
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const graduatedTokens = tokensData.filter(
      (token) => token !== null && token.graduated
    );
    ...
    console.error('Error in graduated-tokens API:', error);
